-- ============================================================================
-- GH-11 · Variante produzione di M11-bis, derivata da
-- supabase/migrations/20260424120000_split_clients_with_backfill.sql.
-- Fonte: docs/incarichi/GH-11-prova-generale-migrazione.md, Fase 2.
--
-- Differenza deliberata: `customers.phone` resta nullable durante la
-- ricostruzione. Il dump reale del 21/8 contiene clienti senza alcuna fonte
-- telefonica deterministica; nessun numero fittizio viene inventato. Il
-- vincolo NOT NULL viene installato solo dall'atto finale separato, dopo la
-- risoluzione esplicita di tutti i casi manuali.
-- ----------------------------------------------------------------------------
...
--     `(tenant_id, phone) WHERE phone IS NOT NULL` durante il backfill.
--
--   • Backfill dei `clients` esistenti via colonna-ponte `pets.legacy_client_id`,
--     dedup conservativo sul telefono normalizzato quando presente,
...
--     sul demo).
--
-- Pre-condition: i clienti senza telefono vengono contati e dichiarati, non
-- scartati. La chiusura della catena resta bloccata finche il conteggio non e
-- zero e `customers.phone` non viene reso NOT NULL.
-- ============================================================================

BEGIN;

-- ============================================================================
-- [0] Pre-flight: misura i clients con phone NULL/vuoto.
-- ============================================================================
DO $$
DECLARE
  v_bad_count integer;
BEGIN
  ...
  IF v_bad_count > 0 THEN
    RAISE NOTICE
      'GH-11 prod variant: % clients senza telefono migrati come customer '
      'temporaneamente nullable; finalizzazione NOT NULL bloccata.',
      v_bad_count;
  END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.customers (
  ...
  phone              text,                                                  -- NOT NULL nell'atto finale GH-11
  marketing_opt_in   boolean NOT NULL DEFAULT false,
  operator_notes     text,
  created_at         timestamptz NOT NULL DEFAULT now(),
  updated_at         timestamptz NOT NULL DEFAULT now(),

  -- Anagrafica completa OR account associato (almeno uno dei due). Il
  -- telefono viene reso obbligatorio dall'atto finale GH-11.
...
-- UNIQUE parziale durante il backfill: i NULL restano esclusi finche i casi
-- manuali non sono risolti. Il vincolo finale conserva la stessa identita
-- tenant+telefono per tutti i valori reali.
...
--     - dedup_key = normalize_phone_it(phone), quando presente
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      -- Stesso telefono, nominativo legacy differente: un solo customer,
      -- tutti i pet preservati e il nominativo alternativo conservato nelle
      -- note operatore senza duplicazioni esatte.
      UPDATE public.customers
      SET operator_notes = CASE
        WHEN lower(regexp_replace(
               btrim(concat_ws(' ', first_name, last_name)), '\s+', ' ', 'g'
             )) = v_dedup_owner
          THEN operator_notes
        WHEN coalesce(operator_notes, '') LIKE
             '%' || '[MIGRATION GH-11] Nominativo legacy aggiuntivo: ' || rec_client.owner || '%'
          THEN operator_notes
        ELSE concat_ws(
          E'\n',
