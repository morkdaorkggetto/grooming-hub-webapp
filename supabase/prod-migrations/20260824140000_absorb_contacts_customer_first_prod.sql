-- GH-13 Atto 2: assorbimento prod-safe di contacts nel modello customer-first.
-- Fonte: docs/incarichi/GH-13-catena-residua-e-fase-3.md e regole GH-07-bis.
-- Nessun tenant, UUID o conteggio demo e hardcoded. Contacts resta disponibile
-- per il periodo di osservazione expand/contract.
...
COMMENT ON FUNCTION public.upsert_customer_lead(
  ...
  'GH-13 prod-safe variant of GH-07-bis. Creates or reuses a customer lead by normalized phone. SECURITY INVOKER with staff guard and tenant-aware RLS.';
...
DECLARE
  ...
  v_total integer;
BEGIN
  SELECT pg_catalog.count(*) INTO v_total FROM public.contacts;

  IF EXISTS (
    SELECT 1
    FROM public.customers c
    GROUP BY c.tenant_id, public.normalize_phone_it(c.phone)
    HAVING pg_catalog.count(*) > 1
  ) THEN
    RAISE EXCEPTION 'GH-13 preflight failed: duplicate normalized customer phones';
  END IF;

  FOR v_contact IN
    SELECT * FROM public.contacts ORDER BY created_at, id
  LOOP
    IF NOT EXISTS (
      SELECT 1 FROM public.tenants t WHERE t.id = v_contact.tenant_id
    ) THEN
      RAISE EXCEPTION 'GH-13 contact % references a missing tenant', v_contact.id;
    END IF;
    ...
    IF v_contact.linked_pet_id IS NOT NULL THEN
      ...
      IF v_linked_customer_id IS NULL THEN
        RAISE EXCEPTION 'GH-13 contact % has an invalid or cross-tenant pet link', v_contact.id;
      END IF;
    END IF;
    ...
    IF COALESCE(pg_catalog.cardinality(v_phone_ids), 0) > 1 THEN
      RAISE EXCEPTION 'GH-13 contact % has ambiguous normalized phone', v_contact.id;
    END IF;
    ...
    IF COALESCE(pg_catalog.cardinality(v_name_ids), 0) > 1 THEN
      RAISE EXCEPTION 'GH-13 contact % has ambiguous assisted name match', v_contact.id;
    END IF;
    ...
    IF v_linked_customer_id IS NOT NULL
      ...
      RAISE EXCEPTION 'GH-13 contact % conflicts between link and phone', v_contact.id;
    END IF;

    IF v_linked_customer_id IS NOT NULL
       AND v_name_ids[1] IS NOT NULL
       AND v_linked_customer_id IS DISTINCT FROM v_name_ids[1] THEN
      RAISE EXCEPTION 'GH-13 contact % conflicts between link and assisted name', v_contact.id;
    END IF;

    IF v_phone_ids[1] IS NOT NULL
       AND v_name_ids[1] IS NOT NULL
       AND v_phone_ids[1] IS DISTINCT FROM v_name_ids[1] THEN
      RAISE EXCEPTION 'GH-13 contact % conflicts between phone and assisted name', v_contact.id;
    END IF;
    ...
    IF v_status IS NULL THEN
      RAISE EXCEPTION 'GH-13 contact % has unsupported status', v_contact.id;
    END IF;

    v_source := COALESCE(NULLIF(pg_catalog.btrim(v_contact.source), ''), 'manual');
    IF v_source NOT IN ('manual', 'whatsapp', 'qr') THEN
      RAISE EXCEPTION 'GH-13 contact % has unsupported source', v_contact.id;
    END IF;

    v_owner_parts := pg_catalog.regexp_split_to_array(
      pg_catalog.btrim(v_contact.owner_name), '\s+'
    );
    IF NULLIF(pg_catalog.btrim(v_contact.owner_name), '') IS NULL THEN
      v_first_name := NULL;
      v_last_name := NULL;
    ELSIF COALESCE(pg_catalog.array_length(v_owner_parts, 1), 0) = 1 THEN
    ...
    IF v_customer_id IS NULL THEN
      IF v_status = 'active'
         ...
         OR v_first_name IS NULL THEN
        RAISE EXCEPTION 'GH-13 contact % has no safe automatic destination', v_contact.id;
      END IF;

      v_marker := CASE
        WHEN NULLIF(pg_catalog.btrim(v_contact.notes), '') IS NULL THEN NULL
        ELSE '[GH-07 contact ' || v_contact.id || '] ' || v_contact.notes
      END;
      ...
      INSERT INTO public.customers (
        ...
        v_source,
        v_status,
        COALESCE(v_contact.created_at, pg_catalog.now())
      )
      RETURNING id INTO v_customer_id;
    ELSE
      UPDATE public.customers c
  ...
  IF v_absorbed <> v_total THEN
    RAISE EXCEPTION 'GH-13 expected % absorptions, got %', v_total, v_absorbed;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'GH-13 backfill produced normalized phone duplicates';
  END IF;
END $$;
...
COMMENT ON FUNCTION public.enforce_customer_directory_fields_staff_only() IS
  'GH-13 prod-safe variant of GH-07-bis. Non-staff cannot modify customer directory fields; row access remains governed by RLS.';
