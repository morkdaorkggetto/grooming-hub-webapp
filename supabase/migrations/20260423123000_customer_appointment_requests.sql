-- ============================================================================
-- CONSOLIDATED BASELINE SNAPSHOT
-- Prodotto via `supabase db dump --schema public` del progetto
-- grooming-hub-demo (ref qttpinkslhenxrsbhhhg), 2026-04-24.
-- Contiene: 8 tabelle public + storage bucket `client-photos` + policy + RPC.
-- Statement DDL totali: 119
--
-- Il timestamp del nome file coincide con l'ultima migration applicata al DB
-- demo prima del pull retrospettivo (20260423123000 customer_appointment_requests).
-- Non modificare il nome: serve a far combaciare `schema_migrations` del DB
-- con la cartella `supabase/migrations/` nel repo.
-- ----------------------------------------------------------------------------
-- Migration history del DB demo ricostruita qui come stub no-op + questo
-- file consolidato:
--   20260305145000 init_schema                         (stub)
--   20260306193000 storage_client_photos               (stub)
--   20260312101500 appointments_and_blacklist          (stub)
--   20260318101500 client_qr_cards                     (stub)
--   20260408093000 public_pet_card_rpc                 (stub)
--   20260409103000 contacts_inbox                      (stub)
--   20260409131500 backfill_contacts_from_clients      (stub)
--   20260422110000 reward_points                       (stub)
--   20260423110000 customer_portal_foundation          (stub)
--   20260423123000 customer_appointment_requests       <-- questo file
--
-- Il dump consolidato è idempotente (`CREATE TABLE IF NOT EXISTS`, `ON CONFLICT
-- DO NOTHING`, `DROP POLICY IF EXISTS`). Uso atteso:
--   - `supabase db reset` su un branch dev/CI vuoto → riapplica tutto da zero.
--   - `supabase db push` sul demo → SKIP perché la history coincide.
--
-- Per capire cosa ha introdotto ogni step storico, vedi
...
-- ============================================================================
-- Storage: bucket `client-photos` + policy (migration 20260306193000
-- `storage_client_photos`). Inclusi qui perché il dump `--schema public` non
-- li cattura. Sono idempotenti.
-- ============================================================================
...
CREATE POLICY "Authenticated users can upload own client photos"
  ON storage.objects
  FOR INSERT
  WITH CHECK (
