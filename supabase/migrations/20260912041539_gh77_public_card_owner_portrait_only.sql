-- GH-77: the public card may show the portrait chosen by the owner, never
-- the salon recognition photo. Source: GH-77 mandate, 2026-09-12.
BEGIN;
...
BEGIN
  ...
  SELECT
    ...
    p.owner_photo_url AS photo_url,
...
COMMENT ON FUNCTION public.get_public_pet_card(text) IS
  'Public QR-card projection. Identity and effective fidelity tier come from the pet tenant; public photo is the owner portrait only; awarded tier origin stays private to staff.';
