-- GH-73: a salon-awarded fidelity tier is independent from configurable
-- thresholds. The customer-facing projection exposes only the effective tier.
-- Source: GH-73 mandate, 2026-09-08.
BEGIN;

ALTER TABLE public.pets
  ADD COLUMN awarded_fidelity_tier text
  CONSTRAINT pets_awarded_fidelity_tier_check
  CHECK (awarded_fidelity_tier IS NULL OR awarded_fidelity_tier = ANY (ARRAY['bronze', 'silver', 'gold']));

COMMENT ON COLUMN public.pets.awarded_fidelity_tier IS
  'GH-73 optional fidelity tier awarded by salon staff; independent from thresholds, visits and reward points.';
...
DECLARE
  ...
  v_awarded_tier_rank integer := 0;
  v_effective_tier_rank integer := 0;
...
BEGIN
  ...
  v_awarded_tier_rank := CASE v_pet.awarded_fidelity_tier
    WHEN 'gold' THEN 3
    WHEN 'silver' THEN 2
    WHEN 'bronze' THEN 1
    ELSE 0
  ...
  v_effective_tier_rank := GREATEST(
    v_visit_tier_rank,
    v_points_tier_rank,
    v_awarded_tier_rank
  );

  CASE v_effective_tier_rank
    WHEN 3 THEN
      v_current_tier := 'gold';
      v_next_tier := NULL;
...
COMMENT ON FUNCTION public.get_public_pet_card(text) IS
  'Public QR-card projection. Identity and effective fidelity tier come from the pet tenant; awarded tier origin stays private to staff.';
