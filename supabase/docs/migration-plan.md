...
| 3 | Split `clients` → `customers` + `pets` come **transazione atomica unica** (un solo file M11 con `BEGIN…COMMIT`). **M11 è demo-only**: `DROP+ADD COLUMN` diretto senza backfill dati (sul demo `clients` ha 0 righe). Per produzione (`grooming`) verrà scritta una migration separata `M11-prod_split_clients_with_backfill.sql` con colonna-ponte `legacy_client_id`, `INSERT`/`UPDATE` dei riferimenti, poi drop. Da rivedere e approvare prima del merge in produzione. |
...
|---|---|---|---|
| 20260424120000 | `20260424120000_split_clients_into_customers_and_pets.sql` | ~340 | **Demo-only.** Crea `customers`, `pets`; drop policy pre-esistenti dipendenti; rewire FK (`visits/appointments/reward_points/contacts/customer_invitations`); drop `customer_client_links` + `clients`; riscrive `get_public_pet_card` e `accept_customer_invite`; pone policy *legacy staff-only* su `visits/reward_points/pets/customers` (sostituite in Fase E). Per produzione serve variante separata `M11-prod_split_clients_with_backfill.sql` (vedi decisione #3). |
...
| 20260424133000 | `20260424133000_tenant_id_enforce_not_null.sql` | Verifica pre-condizione (COUNT righe null = 0), `ALTER COLUMN SET NOT NULL` su tutte le tabelle. Sostituisce `customers.UNIQUE(user_id)` con `UNIQUE(tenant_id, user_id)`. Riscrive `accept_customer_invite` con `ON CONFLICT (tenant_id, user_id)`. |
...
|---|---|---|---|
| 1 | `M11 split_clients_into_customers_and_pets` | `DROP COLUMN client_id; ADD COLUMN pet_id NOT NULL` è distruttivo su dati reali. | Migration separata `M11-prod_split_clients_with_backfill.sql` con colonna-ponte `legacy_client_id`, `INSERT`/`UPDATE` dei riferimenti, poi drop. Vedi decisione #3 e header di M11. |
...
| 6 | `tenant_memberships` vuota dopo M13 | Su demo nessuno ha accesso finché si seedano membership. Su prod: **stesso problema**: dopo M21, tutte le policy RLS della Fase E filtrano per membership, ma nessun utente esistente ne ha una → l'app staff smette di funzionare. | Vedi **"Pattern seed memberships per produzione"** sotto. Mancante nel piano attuale (non scritto in file SQL), va aggiunta al Gate production. |
