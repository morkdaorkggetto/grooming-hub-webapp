...
- **Prova generale della migrazione: GH-11 + GH-12 Atti 1-3 fatti e verificati.** Sul temporaneo `grooming-prova-generale` (`xkieyzuhtpiysjugtdik`): restore prod alle cardinalità esatte, pulizia dei dati di prova, split prod-safe, catena Gate 2 completa, cancellazione delle 8 schede irrecuperabili, `phone NOT NULL` applicato. **Stato attuale misurato: 260 customers, 282 pets, 452 visite, 287 contatti, 0 senza telefono, 0 conflitti.** Cinque file prod-safe versionati in `supabase/prod-migrations/` — la ricetta G6 è riproducibile end-to-end. Restano gli Atti 5-7 (backfill contatti, catena residua, Fase 3): mandato GH-13 da scrivere.
- **Il numero che ancora manca**: la **durata end-to-end** della catena, che dimensiona la finestra di G6. Parziali noti: restore ~27 s, catena Gate 2 36,3 s, drop 1,5 s, `NOT NULL` 1,2 s.
...
**Prossimo passo**: (a) push di Luigi, incluso il README con la regola di sistema; (b) mandato GH-13 per gli Atti 5-7 (backfill contatti, catena residua con preflight `pg_proc`/ACL, Fase 3 con suite RLS, spot-check di 5 clienti veri e **durata end-to-end**); (c) ripulire il §3 Grooming da `BEA_ScuolaMusica/_temp_updates/108-registro-ultimi-task-codex.md`; (d) prima di G6, riautorizzare il connettore Cowork su `Webapp_Project`.
